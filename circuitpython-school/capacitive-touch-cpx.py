# capacitive-touch-cpx.py
# https://youtu.be/eEHOYggqa6U?si=RUVEAbw4Ri_AOPj8

import board, neopixel, digitalio, analogio, touchio
from time import sleep
from adafruit_led_animation.color import BLACK, RED, BLUE, YELLOW, AMBER, GREEN, CYAN
...
colors = [RED, BLUE, YELLOW, AMBER, GREEN, CYAN]
soundfiles = ['splat', 'scratch', 'bd_zome', 'drum_cowbell', 'elec_cymbal', 'elec_hi_snare']

# setup touch pads
touchpads = []
for pin in ([board.A1, board.A2, board.A3, board.A4, board.A5, board.A6]):
...
print("Touch pads A1 -> A6 to play drum sound")
while True:
    for i, touchpad in enumerate(touchpads):
        touchpad.update()
        if touchpad.pressed:
            pixels.fill(colors[i])
            play_sound(soundfiles[i]+".wav")
            pixels.fill(BLACK)
